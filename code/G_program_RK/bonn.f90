module bonn
!Computes the Bonn potential as found in Ref [1] Appendix E.
!Notes
...
!   Ref.[3] = "Computational Nuclear Physics 2: Nuclear Reactions", Editors: K. Langanke et al.
!   In the comments below alpha = pion,eta,sigma,delta,omega,rho

use inputs
implicit none

    !Stores the Bonn potential parameters.The parameters came from Ref.[2] (see chapter 2 page 347).
    ...
    !Coupling constant of pi,eta,sigma,delta,omega,rho mesons in MeV.
    ...
    !n_alpha appering in the form factor. See Ref.[1] p.74.
    ...
    !lambda_alpha appering in the form factor in MeV. See Ref.[1] p.74.
...
subroutine helicity_elements(theta,phi,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_innerprod,lambda_sigma_element)
!Computes <lambda_1' lambda_2' | lambda_1 lambda_2> and 
!   <lambda_1^prime lambda_2^prime | vec{sigma}^(1)*vec{sigma}^(2) |lambda_1 lambda_2>.
!See Ref.[1] Appendix E.

implicit none

    !(theta,phi),(thetap,phip) = Angular momentum variables for vec{q},vec{q'}.
	...
	real(8), intent(in) :: theta,phi,thetap,phip
...
!______________________A few comments about the rest of the routines__________________
!The following is different than what appears in Ref.[1]. We have generilized to include two nucleons of mass m_1 and m_2.
!	    When m_1 = m_2 = m = free space nucleon mass it reduces to the ones found in Ref.[1].
!       In addtion, we ignore the square root in the factor that appears as sqrt(m/E_q)*sqrt(m/E_q').
!	    For example see Eq. (E.33) of Ref. [1]. The reason why we ignore the sqrt is that we will be using the potential in the Thompson equation
!	    (instead of the Blankenbecler and Sugar equation). Finally, instead of the pseudo-scalar Lagrangian, we use the pseudo-vector Lagrangian.

complex(8) function V_pv(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,&
					&lambda1,lambda2,lambda_alpha,n_alpha,g_alpha,m_alpha,m_1,m_2,m)
!Calculates <q' lambda_1' lambda_2' |V_{pv}^{OBE}|q lambda_1 lambda_2>. Derived from pseudo-vector Lagrangian p.14 Ref.[3].
!Used for pi and eta mesons.
!Notes:
...
implicit none
	...
	real(8), intent(in) :: q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_alpha,n_alpha,g_alpha,m_alpha,m_1,m_2,m

    !qp_q_squared = |vec{q'} - vec{q}|**2. It will be used in the form factor.
    !F = Form factor p.74 Ref.[1].
    !E1,E2,E1p,E2p = Generilization of (E1 = E_{q_1},E2 = E_{q_2},E1p = E_{q'_1},E2p = E_{q'_2}). They are the relativistic energies
    !   that appear in the formulas of Ref.[1] p.75-76.
    !W1,W2,W1p,W2p = Generilization of W_1 and W'_1 that appear on p.76 of Ref.[1]. 
	...
	call helicity_elements(theta,phi,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_innerprod,lambda_sigma_element)
...
complex(8) function V_s(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,&
					&lambda1,lambda2,lambda_alpha,n_alpha,g_alpha,m_alpha,m_1,m_2)
!Calculates <q' lambda_1' lambda_2' |V_{s}^{OBE}|q lambda_1 lambda_2>. Derived from the scalar Lagrangian.
!   It's the generilization of Eq.(E.33) in Ref.[1]. Used for sigma and delta mesons.
!Notes:
...
implicit none
	...
	real(8), intent(in) :: q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_alpha,n_alpha,g_alpha,m_alpha,m_1,m_2
...
complex(8) function V_v(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,&
					&lambda1,lambda2,lambda_alpha,n_alpha,g_alpha&
	                &,coupling_ratio_alpha,m_alpha,m_1,m_2,m)
!Calculates <q' lambda_1' lambda_2' |V_{v}^{OBE}|q lambda_1 lambda_2>. Derived from the vector Lagrangian.
!It's the generilization of Eq.(E.34a + E.34b + E.34c) in Ref.[1]. Used for omega and rho mesons.
!Notes:
!   (g_{ps})**2/( (2pi)**3 ) --> x/( 2*pi**2 )  Where x is the coupling constant listed as g_alpha.
!	f_rho/g_rho = 6.1 and f_omega/g_omega = 0. See p.347 of Ref.[2]. 
!	For vector boson exchange, the potential is the sum of three terms (V^{OBE}_v = V^{OBE}_vv + V^{OBE}_tt + V^{OBE}_vt).
...
implicit none
	...
	real(8), intent(in) :: q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_alpha,n_alpha,g_alpha
...
complex(8) function V(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,T)
!Outputs the Bonn potential. V = V^{OBE}_pv + V^{OBE}_s + V^{OBE}_v. It's built from
!   six mesons: (pi,eta) (pseudo-vector),(sigma,delta) (scalar),(omega,rho) (vector).
!Notes:
!   To deal with isospin i.e. the tau dot tau things. Isovector mesons get multiplied by and extra factor of vec{tau} dot vec{tau}
!	where tau^i are the usual Pauli spin matricies. See Ref.[2] p.343.
!	V(T,other variables) = V_eta + V_sigma + V_omega + x*( V_pi + V_delta + V_rho )   Where x = -3 for T = 0 and x = 1 for T = 1.

implicit none

    !T = Total isospin parameter. T can be 0 or 1. This corresponds to the siglet(T=0) or triplet(T=1) state.
    !(q,theta,phi),(qp,thetap,phip) = vec{q},vec{q'} momentum variables.
    !lambda1p,lambda2p,lambda1,lambda2 = < | >,< |sigma^(1)*sigma^(2)| >
    !Note, values for lambda1p,lambda2p,lambda1,lambda2 are 0.5d0 = + and -0.5d0 = - i.e spin up and down.
	integer, intent(in) :: T
	real(8), intent(in) :: q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2

    !V_pi(eta,sigma...) = Different meson contributions to the potential.
    !x = Isospin coefficient. i.e. x = -3 for T = 0 and x = 1 for T = 1.
	complex(8) :: V_pi,V_eta,V_sigma,V_delta,V_omega,V_rho
	real(8) :: x

    !Pi (isovector)
	V_pi = V_pv(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_pi,n_pi,g_pi,m_pi,m_1,m_2,m)

    !Eta (isoscalar)
	V_eta = V_pv(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_eta,n_eta,g_eta,m_eta,m_1,m_2,m)

    !Sigma (isoscalar)
	V_sigma = V_s(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_sigma,n_sigma,g_sigma,m_sigma,m_1,m_2)

    !Delta (isovector)
	V_delta = V_s(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_delta,n_delta,g_delta,m_delta,m_1,m_2)

    !Omega (isoscalar)
	V_omega = V_v(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_omega,n_omega,g_omega&
	&,coupling_ratio_omega,m_omega,m_1,m_2,m)

    !Rho (isovector)
	V_rho = V_v(q,theta,phi,qp,thetap,phip,lambda1p,lambda2p,lambda1,lambda2,lambda_rho,n_rho,g_rho&
